...
const updateLead = async (req, res) => {
    try {
        ...
        if (updateData.disposition === 'SOLD' && req.user?.id) {
            ...
            const sourceCode = updatedLead.sourceHash || updatedLead.sourceCode || updatedLead.source_hash;
            if (sourceCode) {
                try {
                    const success = await (0, sourceCodeQuality_controller_1.autoAssignQuality)(req.user.id, sourceCode, 'quality');
                    if (success) {
