...
async function deleteExclusiveLeads() {
    try {
        ...
        exclusiveLeads.forEach((lead) => {
            console.log(`- ID: ${lead._id}, Name: "${lead.name}", Phone: ${lead.phone}, BidType: ${lead.bidType}`);
        });
        // Delete the leads
        if (exclusiveLeads.length > 0) {
            const result = await Lead_1.default.deleteMany(query);
            console.log(`Successfully deleted ${result.deletedCount} leads with "exclusive" in name or bidType`);
        }
        else {
