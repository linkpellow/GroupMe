...
async function createIndexes() {
    ...
    try {
        ...
        await collection.createIndex({ disposition: 1, createdAt: -1 }, { name: 'disposition_createdAt_compound' });
        // State + Disposition + createdAt (multi-filter pattern)
        await collection.createIndex({ state: 1, disposition: 1, createdAt: -1 }, { name: 'state_disposition_createdAt_compound' });
        // Source + createdAt (pipeline filter)
        await collection.createIndex({ source: 1, createdAt: -1 }, { name: 'source_createdAt_compound' });
