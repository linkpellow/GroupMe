    ...
    try {
        ...
        for (const lead of allLeads) {
            try {
                ...
                else if ((!lead.name || lead.name === 'Unknown') && (lead.firstName || lead.lastName)) {
                    needsUpdate = true;
                }
                ...
                for (const value of bidTypeValues) {
                    if (lead.name?.toLowerCase().includes(value) ||
                ...
                if (needsUpdate) {
                    ...
                    const firstName = fieldChanges.firstName !== undefined ? fieldChanges.firstName : lead.firstName || '';
                    const lastName = fieldChanges.lastName !== undefined ? fieldChanges.lastName : lead.lastName || '';
                    lead.name = `${firstName} ${lastName}`.trim() || lead.name || 'Unknown';
                    await lead.save();
        ...
        console.log(`Finished fixing names. Results: ${fixedCount} fixed, ${errorCount} errors, ${allLeads.length - fixedCount - errorCount} unchanged`);
    }
    catch (error) {
