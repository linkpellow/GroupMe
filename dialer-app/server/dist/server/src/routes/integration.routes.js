...
const ringyService_1 = require("../services/ringyService");
const import_nextgen_leads_1 = require("../scripts/import-nextgen-leads");
const User_1 = __importDefault(require("../models/User"));
const router = (0, express_1.Router)();
// Endpoint to verify API credentials
router.post('/verify', (req, res) => {
    const { sid } = req.body;
    if (sid !== apiConfig_1.API_SID) {
...
router.post('/nextgen', async (req, res) => {
    try {
        ...
        const lastLead = await Lead_1.default.findOne({ assignedTo: adminUser._id }).sort({
            order: -1,
        });
        ...
        for (const lead of leads) {
            try {
                ...
                if (existingLead) {
                    ...
                    savedLead = await Lead_1.default.findByIdAndUpdate(existingLead._id, leadData, {
                        new: true,
                    });
                    ...
                    // Create new lead
                    savedLead = await Lead_1.default.create(leadData);
...
router.post('/import-nextgen', auth_1.auth, async (req, res) => {
    try {
        const result = await (0, import_nextgen_leads_1.importLeads)(req.user?.id);
        res.json(result);
    }
    catch (error) {
...
router.post('/import-ringy', auth_1.auth, async (req, res) => {
    try {
        const adminUser = await User_1.default.findOne({ role: 'admin' });
        if (!adminUser) {
            throw new Error('No admin user found');
        }
        const result = await (0, ringyService_1.importRingyLeads)(adminUser);
        res.json(result);
    }
    catch (error) {
...
router.post('/import-usha', auth_1.auth, async (req, res) => {
    try {
