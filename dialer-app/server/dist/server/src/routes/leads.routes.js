...
const nextgenService_1 = require("../services/nextgenService");
...
const headerAliases_1 = require("../utils/headerAliases");
const csvParser_1 = require("../utils/csvParser");
const fixDob_1 = require("../utils/fixDob");
...
const leadValidation = [
    (0, express_validator_1.check)('name', 'Name is required').notEmpty().optional({ nullable: true }),
    (0, express_validator_1.check)('phone', 'Phone number is required').optional({ nullable: true }),
    (0, express_validator_1.check)('email', 'Please include a valid email').isEmail().optional({ nullable: true }),
    (0, express_validator_1.check)('status', 'Status is required')
        .isIn(['New', 'Contacted', 'Follow-up', 'Won', 'Lost'])
        .optional({ nullable: true }),
    (0, express_validator_1.check)('source', 'Invalid source provided')
...
router.get('/test-db', auth_1.auth, leadsController.testDb);
// Main GET route for fetching leads with query parameters
router.get('/', auth_1.auth, validateQuery_middleware_1.validateQueryMiddleware, validateQuery_middleware_1.rateLimitMiddleware, (req, res, next) => leadsController.getLeads(req, res));
...
const classifyRow = (row) => {
    ...
    if (row.customer_id || // Key indicator for NextGen CSV format
    ...
    if (row.vendor_name?.toString().toLowerCase().includes('marketplace') ||
...
const normaliseRow = (raw) => {
    ...
    const isLikelyNextGen = raw.customer_id !== undefined || (raw.first_name !== undefined && raw.last_name !== undefined);
    ...
    Object.entries(raw).forEach(([k, v]) => {
        ...
        const canon = headerAliases_1.HEADER_ALIASES[k.trim()] || headerAliases_1.HEADER_ALIASES[k.trim().toLowerCase()] || k;
    ...
    if (out.dob && typeof out.dob === 'string') {
        const fixed = (0, fixDob_1.fixDob)(out.dob);
        if (fixed) {
            out.dob = fixed;
...
router.post('/upload', auth_1.auth, upload.single('file'), async (req, res) => {
    try {
        ...
        const maxOrderLead = await Lead_1.default.findOne().sort({ order: -1 });
            ...
            for (const rawRow of batch) {
                try {
                    ...
                    if (isNextGenLead) {
                        ...
                        console.log(`NextGen lead name parts: firstName="${firstName}", lastName="${lastName}"`);
                    }
                    ...
                    console.log(`Phone field mapping - Raw: ${JSON.stringify({
                        phone: row.phone,
                        Phone: row.Phone,
                        PHONE: row.PHONE,
                    ...
                    if (existingLead) {
                        console.log(`Found existing lead: ${existingLead.name} (${existingLead.phone || existingLead.email})`);
                        ...
                        const updatedData = {
                            ...
                            name: existingLead.name === 'Unknown' || !existingLead.name
                                ? fullName
                                : existingLead.name,
                        ...
                        await Lead_1.default.updateOne({ _id: existingLead._id }, { $set: updatedData });
                        stats.updated++;
                        ...
                        const lead = new Lead_1.default({
                            ...
                            zipcode: row.zipcode || row.Zipcode || row.zip || row.Zip || row.ZIP || row.zip_code || '',
                            dob: row.dob ||
                                row.DOB ||
                                row.dateOfBirth ||
                                row.date_of_birth ||
...
router.post('/import-csv', auth_1.auth, upload.single('file'), async (req, res) => {
    ...
    try {
        ...
        const parseResult = await (0, csvParser_1.parseVendorCSV)(fileContent, {
            skipEmptyLines: true,
        ...
        if (parseResult.vendor === 'UNKNOWN') {
            ...
            if (!manualSource) {
                return res.status(400).json({
                    ...
                    suggestion: "Please specify 'source' in the request body with one of: " + validSources.join(', '),
                });
            }
        }
        ...
        if (!sourceToUse && parseResult.vendor !== 'UNKNOWN') {
            sourceToUse = (0, csvParser_1.getVendorDisplayName)(parseResult.vendor);
        }
        if (!sourceToUse) {
            sourceToUse = 'CSV Import'; // Default fallback
        }
        console.log(`Using source: ${sourceToUse} (Manual: ${!!manualSource}, Detected: ${(0, csvParser_1.getVendorDisplayName)(parseResult.vendor)})`);
        ...
        for (const leadData of parseResult.leads) {
            try {
                ...
                const existingLead = await Lead_1.default.findOne({
                    $or: [{ phone: leadData.phone }, { email: leadData.email }].filter((condition) => condition.phone || condition.email),
                });
                if (existingLead) {
                    // Update existing lead only if source matches or no source set
                    if (!existingLead.source || existingLead.source === leadData.source) {
                        await Lead_1.default.findByIdAndUpdate(existingLead._id, {
                        ...
                        skipped++;
                        console.log(`Skipped lead with mismatched source: ${leadData.name} (existing: ${existingLead.source}, new: ${leadData.source})`);
                    }
                }
                else {
...
router.post('/import-csv-now', upload.single('file'), async (req, res) => {
    try {
        if (!req.file) {
            return res.status(400).json({ error: 'No file uploaded' });
        }
        ...
        res.json({
            success: true,
            vendor: (0, csvParser_1.getVendorDisplayName)(parseResult.vendor),
            imported: count,
...
router.get('/call-counts', auth_1.auth, leadsController.getCallCounts);
// Fetch Usha leads - requires admin
router.get('/sync-usha', auth_1.auth, auth_1.isAdmin, async (req, res) => {
    try {
        const result = await (0, ushaService_1.fetchUshaLeads)();
        res.json(result);
    }
    catch (error) {
...
router.get('/sync-nextgen', auth_1.auth, auth_1.isAdmin, async (req, res) => {
    try {
        const result = await (0, nextgenService_1.syncNextGenLeads)();
        res.json(result);
    }
    catch (error) {
...
// Create new lead
router.post('/', [auth_1.auth, auth_1.isAdmin, ...leadValidation], leadsController.createLead);
// Update lead
router.put('/:id', [auth_1.auth], leadsController.updateLead);
...
router.delete('/delete-placeholders', auth_1.auth, async (req, res) => {
    try {
        // Delete leads where name is "Unknown Name" or empty
        const result = await Lead_1.default.deleteMany({
...
router.post('/:id/reorder', auth_1.auth, leadsController.reorderLead);
// New route for reversing the order
router.post('/reverse-order', auth_1.auth, leadsController.reverseOrder);
// Update leads with missing fields from CSV
router.post('/update-leads-data', auth_1.auth, auth_1.isAdmin, async (req, res) => {
    try {
        ...
        for (const record of records) {
            try {
                ...
                const lead = await Lead_1.default.findOne({ phone: record.phone });
                if (lead) {
...
router.get('/reconcile-dispositions', auth_1.auth, auth_1.isAdmin, async (req, res) => {
    try {
        // Find leads with non-empty dispositions
        const leadsWithDispositions = await Lead_1.default.find({
        ...
        const leadsWithoutDispositions = await Lead_1.default.find({
        ...
        for (const lead of leadsWithoutDispositions) {
            ...
            if (inferredDisposition) {
                console.log(`Setting disposition "${inferredDisposition}" for lead ${lead._id} based on notes`);
                await Lead_1.default.updateOne({ _id: lead._id }, { $set: { disposition: inferredDisposition } });
                updates++;
            }
        }
        // Return the results
...
router.get('/fix-csv-duplicates', auth_1.auth, auth_1.isAdmin, async (req, res) => {
    try {
        console.log('Starting CSV duplicates fix process...');
        // 1. Identify all leads, grouping by phone or email
        const allLeads = await Lead_1.default.find({}).lean();
        // Group by phone
        ...
        // Identify duplicates
        const duplicatePhones = Array.from(phoneGroups.entries()).filter(([phone, leads]) => leads.length > 1);
        const duplicateEmails = Array.from(emailGroups.entries()).filter(([email, leads]) => leads.length > 1);
        ...
        for (const [phone, leads] of duplicatePhones) {
            ...
            for (const newerLead of newerLeads) {
                ...
                if (newerLead.disposition && (!oldestLead.disposition || oldestLead.disposition === '')) {
                    await Lead_1.default.updateOne({ _id: oldestLead._id }, { $set: { disposition: newerLead.disposition } });
                    stats.dispositionsRecovered++;
                    console.log(`Recovered disposition "${newerLead.disposition}" from duplicate to lead ${oldestLead._id}`);
                }
                ...
                if (newerLead.notes && newerLead.notes !== oldestLead.notes) {
                    const combinedNotes = oldestLead.notes
                        ...
                        : newerLead.notes;
                    await Lead_1.default.updateOne({ _id: oldestLead._id }, { $set: { notes: combinedNotes } });
        ...
        res.json({
            ...
            stats,
            action: 'Dispositions and notes have been recovered from duplicates. Duplicates were NOT deleted. Review and run again with deletion enabled when ready.',
        });
    }
    catch (error) {
...
router.post('/record-call', auth_1.auth, async (req, res) => {
    try {
...
router.post('/fix-all-leads', auth_1.auth, async (req, res) => {
    try {
        ...
        for (const lead of leads) {
            try {
                ...
                const isMarketplaceLead = (lead.source && lead.source.toLowerCase().includes('marketplace')) ||
                ...
                const isNextGenLead = (lead.source && lead.source.toLowerCase().includes('nextgen')) ||
                ...
                else if ((!lead.name || lead.name === 'Unknown') &&
                    !lead.firstName &&
                    !lead.lastName &&
                    !lead.email) {
                ...
                if (shouldUpdate) {
                    await Lead_1.default.updateOne({ _id: lead._id }, { $set: updates });
                    updatedCount++;
...
router.get('/filters', auth_1.auth, leadsController.getFilterOptions);
