...
const sourceCodeQuality_routes_1 = __importDefault(require("./routes/sourceCodeQuality.routes"));
...
const broadcastNewLeadNotification = (leadData) => {
    ...
    logger_1.default.info(`Broadcasting lead notification: ${leadData.name} (${leadData.leadId || 'no-id'}), isNew=${leadData.isNew}, clients=${wss.clients.size}`);
    let sentCount = 0;
    wss.clients.forEach((client) => {
        if (client.readyState === ws_1.WebSocket.OPEN) {
            try {
...
app.use('/api/source-code-quality', sourceCodeQuality_routes_1.default);
