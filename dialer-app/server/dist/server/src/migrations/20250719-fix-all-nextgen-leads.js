...
async function fixAllNextGenLeads() {
    console.log('🔄 Starting comprehensive NextGen sourceCode migration...\n');
    try {
        // Connect to MongoDB
        await mongoose_1.default.connect(process.env.MONGODB_URI);
        console.log('✅ Connected to MongoDB');
        // Find ALL NextGen leads
        const allNextGenLeads = await Lead_1.default.find({
            source: 'NextGen'
        ...
        for (const lead of allNextGenLeads) {
            ...
            if (hasValidSourceCode) {
                console.log(`✅ Lead ${lead.name} already has valid sourceCode: "${lead.sourceCode}"`);
                skipCount++;
                continue;
            }
            ...
            if (lead.sourceHash && lead.sourceHash.trim() !== '') {
                ...
                console.log(`  📋 Using sourceHash: "${newSourceCode}"`);
            }
            // Strategy 2: Extract from notes (webhook data often stored there)
                ...
                if (hashMatch) {
                    ...
                    console.log(`  📋 Extracted from notes: "${newSourceCode}"`);
                }
            }
            // Strategy 3: Generate based on creation date (fallback)
            else {
                // Use a deterministic hash based on lead ID for consistency
                const leadIdHash = lead._id.toString().slice(-6);
                newSourceCode = `NG${leadIdHash}`;
                console.log(`  📋 Generated fallback: "${newSourceCode}"`);
            }
            if (newSourceCode) {
                // Update the lead
                await Lead_1.default.updateOne({ _id: lead._id }, {
                    $set: {
                        sourceCode: newSourceCode,
                        // Also set sourceHash if it was missing
                        ...((!lead.sourceHash || lead.sourceHash.trim() === '') && { sourceHash: newSourceCode })
                    }
                });
                console.log(`  ✅ Updated sourceCode to: "${newSourceCode}"`);
                updateCount++;
            }
            else {
                console.log(`  ⚠️  Could not determine sourceCode for lead`);
                skipCount++;
            }
        }
        console.log('\n📊 Migration Summary:');
        ...
        const fixedLeads = await Lead_1.default.countDocuments({
            source: 'NextGen',
            sourceCode: { $exists: true, $ne: null, $ne: '', $ne: 'NextGen' }
        });
        console.log(`\n✅ Verification: ${fixedLeads} NextGen leads now have proper sourceCode`);
