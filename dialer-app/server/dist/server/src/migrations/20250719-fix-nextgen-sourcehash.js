...
async function fixNextGenSourceCodes() {
    ...
    try {
        ...
        for (const lead of leads) {
            ...
            await Lead_1.default.updateOne({ _id: lead._id }, { $set: { sourceCode: lead.sourceHash } });
        ...
        const leadsWithoutSourceCode = await Lead_1.default.countDocuments({
            source: 'NextGen',
        ...
        if (process.argv.includes('--fix-defaults')) {
            ...
            const result = await Lead_1.default.updateMany({
                source: 'NextGen',
                sourceCode: { $in: [null, '', 'NextGen'] },
                sourceHash: { $exists: true, $ne: null },
                $and: [{ sourceHash: { $ne: '' } }]
