import {
  ...
  isMainDataRecord,
  getDeduplicationKey,
  formatPremiumListingNote,
  processNextGenBatch
} from '../src/services/nextgenDeduplicationService';

describe('NextGen Deduplication Service', () => {
  describe('isPremiumListing', () => {
    it('should identify premium listings by product="ad"', () => {
      expect(isPremiumListing({ product: 'ad' })).toBe(true);
      expect(isPremiumListing({ product: 'data' })).toBe(false);
      expect(isPremiumListing({ product: 'other' })).toBe(false);
      expect(isPremiumListing({})).toBe(false);
    });
  });

  describe('isMainDataRecord', () => {
    it('should identify main data records by product="data"', () => {
      expect(isMainDataRecord({ product: 'data' })).toBe(true);
      expect(isMainDataRecord({ product: 'ad' })).toBe(false);
      expect(isMainDataRecord({ product: 'other' })).toBe(false);
      expect(isMainDataRecord({})).toBe(false);
    });
  });

  describe('getDeduplicationKey', () => {
    it('should prioritize leadId > nextgenId > phone > email', () => {
      expect(getDeduplicationKey({ leadId: 'L123', nextgenId: 'N456', phone: '555-1234', email: 'test@example.com' }))
        .toBe('L123');
      expect(getDeduplicationKey({ nextgenId: 'N456', phone: '555-1234', email: 'test@example.com' }))
        .toBe('N456');
      expect(getDeduplicationKey({ phone: '555-1234', email: 'test@example.com' }))
        .toBe('555-1234');
      expect(getDeduplicationKey({ email: 'test@example.com' }))
        .toBe('test@example.com');
      expect(getDeduplicationKey({})).toBe(null);
    });
  });

  describe('formatPremiumListingNote', () => {
    it('should format premium listing note correctly', () => {
      const note = formatPremiumListingNote({ base: 45, premium: 5, total: 50 });
  ...
  describe('processNextGenLead', () => {
    ...
    it('should create new lead when no existing lead', () => {
      const result = processNextGenLead(baseLead, undefined);
    ...
    it('should merge premium listing with existing lead', () => {
      ...
      const result = processNextGenLead(premiumLead, existingLead);
    ...
    it('should replace premium listing with main data record', () => {
      ...
      const result = processNextGenLead(baseLead, existingPremium);
    ...
    it('should handle duplicate main records', () => {
      ...
      const result = processNextGenLead(newMain, existingMain);
  ...
  describe('processNextGenBatch', () => {
    it('should deduplicate batch of leads correctly', () => {
      ...
      const result = processNextGenBatch(leads);
