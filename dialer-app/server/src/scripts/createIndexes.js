...
async function createIndexes() {
  ...
  try {
    ...
    console.log(
      "2. For disposition filtering: use disposition_createdAt_compound",
    );
    console.log(
      "3. For combined filters: use state_disposition_createdAt_compound",
    );
    console.log("4. For text search: use text_search_index");
    console.log(
      "5. For exact email/phone lookup: use email_partial or phone_partial",
    );
...
if (require.main === module) {
  const MONGODB_URI =
    process.env.MONGODB_URI || "mongodb://localhost:27017/crokodial";
  mongoose_1.default
    .connect(MONGODB_URI)
    .then(() => {
