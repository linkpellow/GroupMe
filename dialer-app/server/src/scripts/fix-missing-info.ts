    ...
    for (const lead of leads) {
      try {
        ...
        if (lead.notes && lead.notes.includes('Location:')) {
          const locationMatch = lead.notes.match(/Location: ([^,]+), ([A-Z]{2}) (\d{5})/);
          if (locationMatch) {
        ...
        if (lead.notes && lead.notes.includes('{')) {
          try {
            ...
            if (jsonMatch) {
              ...
              if (!lead.city && jsonData.city) {
                lead.city = jsonData.city;
              }
              if (!lead.state && jsonData.state) {
                lead.state = jsonData.state;
              }
              if (!lead.zipcode && jsonData.zip_code) {
                lead.zipcode = jsonData.zip_code;
              }
              if (!lead.dob && jsonData.birthday) {
                lead.dob = jsonData.birthday;
              }
