...
async function fixNameFields() {
  try {
    ...
    const badLeads = await Lead_1.default.find({
      $or: [
        { firstName: "exclusive" },
        { firstName: "Exclusive" },
    ...
    for (const lead of badLeads) {
      try {
        ...
        const isPhoneActuallyLastName = lead.phone
          ? !lead.phone.match(/\d{10}/) &&
            !lead.phone.match(/\d{3}-\d{3}-\d{4}/)
          : false;
    ...
    console.log(
      `Finished fixing names. Results: ${fixedCount} fixed, ${errorCount} errors`,
    );
  } catch (error) {
