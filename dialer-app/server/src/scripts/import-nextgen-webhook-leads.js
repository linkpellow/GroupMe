...
async function importWebhookLeads() {
  try {
    ...
    const lastLead = await Lead_1.default
      .findOne({ assignedTo: admin._id })
      .sort({ order: -1 });
    ...
    for (const lead of leads) {
      try {
        ...
        const leadData = {
          ...
          dob: webhookData.dob
            ? new Date(webhookData.dob).toLocaleDateString()
            : "",
          ...
          gender: webhookData.gender
            ? webhookData.gender.charAt(0).toUpperCase() +
              webhookData.gender.slice(1)
            : "",
          order: nextOrder++,
        ...
        // Update the lead
        await Lead_1.default.findByIdAndUpdate(lead._id, leadData, {
          new: true,
        });
