  ...
  try {
    ...
    for (const lead of badLeads) {
      try {
        ...
        console.log(
          `Processing lead: ${lead.name}, current values:`,
          origValues,
        );
        // Use mongoose model directly instead of db.collection
        const updateResult = await Lead_1.default.updateOne(
          { _id: lead._id },
          {
            $set: {
              lastName: lead.phone,
        ...
        console.error(
          `Error fixing lead ${lead._id instanceof mongoose_1.default.Types.ObjectId ? lead._id.toString() : String(lead._id)}:`,
          error,
        );
        errorCount++;
      }
...
async function fixPhoneNumbersFromNotes() {
  ...
  try {
    ...
    console.log(
      `Found ${leads.length} leads with potential phone number issues`,
    );
    for (const lead of leads) {
      try {
        ...
        if (lead.notes) {
          try {
            ...
            if (
              ...
              console.log(
                `Found phone number ${phoneNumber} for lead ${lead.name}`,
              );
              ...
              if (updateResult.modifiedCount > 0) {
                console.log(
                  `Updated phone for lead ${lead.name} to ${phoneNumber}`,
                );
                fixedCount++;
              }
            }
        ...
        console.error(
          `Error fixing phone for lead ${lead._id instanceof mongoose_1.default.Types.ObjectId ? lead._id.toString() : String(lead._id)}:`,
          error,
        );
      }
    }
