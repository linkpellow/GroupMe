  ...
  try {
    ...
    for (const lead of badLeads) {
      try {
        ...
        const updateResult = await LeadModel.updateOne(
          { _id: lead._id as mongoose.Types.ObjectId },
          {
            $set: {
              lastName: lead.phone,
        ...
        console.error(
          `Error fixing lead ${lead._id instanceof mongoose.Types.ObjectId ? lead._id.toString() : String(lead._id)}:`,
          error
        );
        errorCount++;
      }
  ...
  try {
    ...
    for (const lead of leads) {
      try {
        ...
        console.error(
          `Error fixing phone for lead ${lead._id instanceof mongoose.Types.ObjectId ? lead._id.toString() : String(lead._id)}:`,
          error
        );
      }
    }
